...
# Input ----
dat_1 <-
  read.csv(paste0(path_root, "output/", "catch_no_sop_year_ctry_1989_", year_data, ".csv"))
dat_1 <- subset(dat_1, year >= 1991)
...
t1 <- flextable(dat_1_t) %>%
  ...
  set_caption(
    caption = paste0(
      "Table 3.1.1 WESTERN BALTIC HERRING. Both WBSS and NSAS. Total catch in 1991-",
      year_data,
...
fleet_map <- arrange(distinct(fleet_map, area, subFleet, fleet), area, subFleet, fleet)

fleet_map <- rename(fleet_map, "sub-fleet" = "subFleet")
...
t2 <- flextable(fleet_map) %>%
  ...
  set_caption(
    caption = paste0(
      "Table 3.1.2 WESTERN BALTIC HERRING. Map bwtween data submission fleets (sub-fleet) and HAWG fleets"
    ),
    style = "Table Caption"
  ) 
...
```{r t_3_1_3, ft.align="left", eval = T}
...
t2 <- flextable(dat_2_t) %>%
  ...
  set_caption(
    caption = paste0(
      "Table 3.1.3 WESTERN BALTIC HERRING. Both WBSS and NSAS. Catch (SOP, 1000 tonnes) in 2000-",
      year_data,
...
canumFS <-
  readRDS(paste(
    path_root,
    "data/",
    "21_C4_her2024_canum_wbss_nsas_",
    year_data,
    ".rds",
    sep = ""
  ))
...
data_321 <-
  aggregate(
    cbind(caton_t) ~ year + area + quarter + ctry + noSample +
      noLength + noAge + fleet + subFleet,
    data = canumF1,
    FUN = sum
  )
data_321 <- rename(data_321, "SOP_1000t" = caton_t, "noSample" = noSample)
data_321 <-
  aggregate(
...
dat_322 <-
  summarise(
    group_by(dat_322, year, area, ctry, quarter, subFleet, level_used),
    catch_t = round(sum(catch_t, na.rm = F), digits = 1)
  )
dat_322 <- arrange(dat_322, year, area, ctry, subFleet, quarter)

dat_322 <-
  select(dat_322, year, area, quarter, ctry, subFleet, catch_t, level_used)

dat_322 <- rename(dat_322, `data used` = level_used, 
    `sub-fleet` = subFleet)
