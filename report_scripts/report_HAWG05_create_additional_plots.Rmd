...
dir_data_input <- "https://www.stockassessment.org/datadisk/stockassessment/userdirs/user295/WBSS_HAWG_2026/data/" #input data directly from www.stockassessment.org

# output_path <- "report_script"

source("../report_scripts/ggplot_AgeDistStk.R")

load("../boot/data/download_from_stockassessment_org_single_fleet/model.RData")
...
ggplot(com, aes(x=X0, y=X1, label=substr(year,3,4))) +
  ...
  geom_text(x = 9000, y = 500, label = lm_eqn(com, x=com$X0, y=com$X1), parse = TRUE) +
...
cw <- as.data.frame(cw)
cw$year <- row.names(cw)
cw2 <- gather(cw, key = "wr", value = "weca", -year)

ggplot(cw2, aes(year, weca, colour = as.character(wr), group = as.character(wr))) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_colour_manual(values = viridis(9, option = "D"), name = "wr") +
  labs(title = "Mean weight at age in catch", y = "WECA (kg)") + 
...
sw2 <- gather(sw, key = "wr", value = "weca", -year)

ggplot(sw2, aes(year, weca, colour = as.character(wr), group = as.character(wr))) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_colour_manual(values = viridis(9, option = "D"), name = "wr") +
  labs(title = "Mean weight at age in stock", y = "WECA (kg)") + 
  guides(colour = guide_legend(reverse = F, nrow = 1)) +
...
f1 <- faytable(fit)
f1 <- as.data.frame(f1)
f1$year <- row.names(f1)
fc1 <- gather(f1, key = "wr", value = "f", -year)

ggplot(fc1, aes(year, f, colour = as.character(wr), group = as.character(wr))) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_colour_manual(values =  viridis(9, option = "D"), name = "wr") +
  labs(y = "f") + 
...
cn <- as.data.frame(cn)
cn$year <- row.names(cn)
cn2 <- gather(cn, key = "wr", value = "n", -year)
...
cn5<-gather(cn4, key = "variable", value = "value", -year, -wr)
cn5<-arrange(cn5, -as.numeric(wr))
cn5$wr<-as.factor(cn5$wr)
cn5$wr <- factor(cn5$wr, levels = rev(levels(cn5$wr)))
levels(cn5$variable) <- c("Number (millions)", "Proportion")

ggplot(cn5, aes(x = year, y = value, fill=wr, group = wr)) +
...
cn <- round(read.ices(paste(dir_data_input, "cn.dat", sep = "")), digits =
              4)
cw <- round(read.ices(paste(dir_data_input, "cw.dat", sep = "")), digits =
              4)
w <- cn * cw
w <- as.data.frame(w)
w$year <- row.names(w)
w2 <- gather(w, key = "wr", value = "w", -year)
w2$w <- w2$w / 1000
...
w5 <- gather(w4, key = "variable", value = "value", -year, -wr)
w5 <- arrange(w5, -as.numeric(wr))
w5$wr <- as.factor(w5$wr)
w5$wr <- factor(w5$wr, levels = rev(levels(w5$wr)))
levels(w5$variable) <- c("Number (millions)", "Proportion")
...
ggplot(w5, aes(
  ...
  facet_grid(variable ~ ., scales = "free_y") +
  scale_fill_manual(values =  rev(viridis(9, option = "D")), name = "wr") +
  labs(title = "Catch in weight", y = "") + guides(colour = guide_legend(reverse = T)) +
  theme(axis.text.x = element_text(
    angle = 90,
    vjust = 0.5,
    hjust = 1
...
ggplot(faa, aes(age,val,group=year)) + geom_line(aes(colour = csum)) + facet_wrap(~grp, scales="free_y") + labs(y="Catchability", x="wr") + scale_colour_gradientn(colours = viridis(7), name="Order")
```
