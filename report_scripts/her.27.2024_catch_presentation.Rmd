...
imp_for_overview <- distinct(imp, ctry, year, area, fleet, subFleet, quarter, catch_t, 
...
```{r}
lan_sum <- summarise(group_by(imp, year), canum_1000 = sum(canum_1000))
...
```{r}
lan_sum <- summarise(group_by(imp, year, area), canum_1000 = sum(canum_1000))
lan_sum_t <- tidyr::spread(lan_sum, key = year, value = canum_1000, fill = 0)
...
```{r}
imp$division <- substr(imp$area, 1, 6)
lan_sum <- summarise(group_by(imp, year, division), canum_1000 = sum(canum_1000))
lan_sum_t <- tidyr::spread(lan_sum, key = year, value = canum_1000, fill = 0)
...
```{r}
lan_sum <- summarise(group_by(imp, year, fleet), canum_1000 = sum(canum_1000))
lan_sum_t <- tidyr::spread(lan_sum, key = year, value = canum_1000, fill = 0)
...
```{r}
lan_sum <- summarise(group_by(imp, year, quarter), canum_1000 = sum(canum_1000))
lan_sum_t <- tidyr::spread(lan_sum, key = year, value = canum_1000, fill = 0)
