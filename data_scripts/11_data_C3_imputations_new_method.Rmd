...
year_impute <- 2025
years_time_series <- c(2000:year_impute)
...
dat <-  readRDS(paste0("../data/C2_her2024_canum_without_imputations_2000_2025.rds"))
...
check_fleets <- arrange(distinct(subset(dat, year == year_impute), year, ctry, area, fleet, subFleet), area, fleet, subFleet)
```
...
dat_without_bio <- subset(dat, !(id %in% dat_with_bio_0$id) & year == year_impute)
...
no_years <- length(years_time_series) - 1
...
# Not relevant for a normal assessement year
...
# Define levels
## Level 295 - this depends on level_now above, so level_now + 1

dat_with_bio$level_157 <- paste(
  dat_with_bio$year,
  dat_with_bio$quarter,
  dat_with_bio$area,
  dat_with_bio$fleet,
...
dat_without_bio$level_157 <- paste(
  dat_without_bio$year,
  dat_without_bio$quarter,
  dat_without_bio$area,
  dat_without_bio$fleet,
...
dat_without_bio$level_158 <- paste(
  dat_without_bio$year_impu_v1,
  dat_without_bio$quarter_impu_v1,
  dat_without_bio$area,
  dat_without_bio$fleet,
...
dat_without_bio$level_159 <- paste(
  dat_without_bio$year,
  dat_without_bio$quarter_impu_v2,
  dat_without_bio$area,
  dat_without_bio$fleet,
...
dat_without_bio$level_160 <- paste(
  dat_without_bio$year,
  dat_without_bio$quarter,
  dat_without_bio$area_impu,
  dat_without_bio$fleet,
...
dat_without_bio$level_161 <- paste(
  dat_without_bio$year_impu_v1,
  dat_without_bio$quarter_impu_v1,
  dat_without_bio$area_impu,
  dat_without_bio$fleet,
...
dat_without_bio$level_162 <- paste(
  dat_without_bio$year,
  dat_without_bio$quarter_impu_v2,
  dat_without_bio$area_impu,
  dat_without_bio$fleet,
...
write.table(dat_with_bio, paste0("../data/11_her2024_canum_with_samples_", min(years_time_series), "_", year_impute, ".csv"), row.names = F, sep = ";")
...
# In 2025 Poland has a large amount of by-catch (active < 32mm) in area 24, quarter 4
# The automated loop assign a Danish sample from the sandeel fishery (with a single herring)  
# It was believed that a Polish sample from 2024 would represent the fishery better
# So here we go ......

dat_with_bio_manual <- subset(dat_with_bio, year == 2024 &
                                ctry == "Poland" & 
                                area == "27.3.d.24" &
                                subFleet == "active < 32mm" &
                                quarter == 4)

unique(dat_with_bio_manual$level_1)

dat_with_bio$level_999[dat_with_bio$year == 2024 &
                       dat_with_bio$ctry == "Poland" &
                       dat_with_bio$area == "27.3.d.24" &
                       dat_with_bio$subFleet == "active < 32mm" &
                       dat_with_bio$quarter == 4] <- unique(dat_with_bio_manual$level_1)

dat_without_bio$level_999[dat_without_bio$year == 2025 &
                       dat_without_bio$ctry == "Poland" &
                       dat_without_bio$area == "27.3.d.24" &
                       dat_without_bio$subFleet == "active < 32mm" &
                       dat_without_bio$quarter == 4] <- unique(dat_with_bio_manual$level_1)

unique(dat_with_bio$level_999)
unique(dat_without_bio$level_999)
...
for (i in c(999, 1:162)) {
  ...
  dat_with_bio_here <- subset(dat_with_bio, !(subFleet %in% c("C", "D", "F")) & 
                                !(is.na(subFleet)) & !(is.na(level_here)))
...
comb <- subset(comb, year == year_impute)

dat_imputed$tons <- round(dat_imputed$catch_t, 0)

dat_with_bio <- subset(dat_with_bio, year == year_impute)
...
dat_imputed$wr[dat_imputed$wr == "8+"] <- 8
...
dat_imputed_sum <- summarise(group_by(dat_imputed, ctry, year, area, fleet, subFleet, 
                                      ...
                                      noAge, level, level_used, catch_t),
                                      weca_g = sum(canum_1000 * weca_g, na.rm = T) / sum(canum_1000, na.rm = T),
                             canum_1000 = sum(canum_1000, na.rm = T))
...
```{r}
dat_done <- bind_rows(dat_with_bio, dat_imputed_sum)
```

# CATON check

```{r}
caton_start <- distinct(dat, ctry, year, quarter, area, fleet, subFleet, 
                                      quarter, catch_t)

caton_end <- distinct(dat_done, ctry, year, quarter, area, fleet, subFleet, 
                                      quarter, catch_t)
...
sop <- summarise(group_by(dat_imputed_sum, ctry, sppName, year, area, 
              quarter, fleet, subFleet, catch_t),
              sop = sum((canum_1000 * weca_g) / 1000, na.rm = T))


dat_imputed_sop <- left_join(select(dat_imputed_sum), sop)
dat_imputed_sop$sopCorr <- (dat_imputed_sop$catch_t / dat_imputed_sop$sop)

nrow(subset(dat_imputed_sop, sopCorr != 1))
...
write.table(dat_done, paste0("../data/11_her2024_canum_with_imputations_", year_impute, ".csv"), row.names = F, sep = ";")
...
write.table(dat_split_sum, paste0("../data/11_her2024_canum_with_imputations_for_split_", year_impute,".csv"), row.names = F, sep = ";")
